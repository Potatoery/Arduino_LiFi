...
void buffer_ber_test(){
  for (int i = 0; i < strlen(ber_string) + SYNCBYTE + 1; i++) {
    if(i == SYNCBYTE - 1){
      for (int j = 0; j < 8; j++) {
        if(_symbol_SOT[7 - j]){
          bitSet(string_Signal[i], j);
        }else{
          bitClear(string_Signal[i], j);
        }
      }
...
void buffer_image(){
  ...
  // bmpImage.seek( );
  if(size > packet_size){
    size_divider = size / packet_size + 1;
    size_divider_n = size % packet_size;
  }
  ...
  for (uint16_t i = 0; i < block_size + SYNCBYTE + 3; i++) {
    if(i == SYNCBYTE - 1){
    ...
    } else if(i == (packet_size + SYNCBYTE + 2)){
      for (int j = 0; j < 8; j++) {
        if(_symbol_EOT[7 - j]){
          bitSet(string_Signal[i], j);
        }else{
      ...
      for (int j = 0; j < 8; j++) {
        if(bitRead(pgm_read_byte_near(image + (image_index * packet_size) + (i - SYNCBYTE - 2)), j)){
          bitSet(string_Signal[i], j);
        }else{
          bitClear(string_Signal[i], j);
        }
